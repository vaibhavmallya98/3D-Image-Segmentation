...
class ResidualBlockEncDec(nn.Module):
    
    def __init__(self, in_channels, out_channels):
        super(ResidualBlockEncDec, self).__init__()
        groups = min(in_channels, out_channels)

        self.conv = nn.Sequential(nn.Conv3d(in_channels, out_channels, 3, stride=1, padding=1, groups=groups),
                                    Norm(dim=out_channels),
                                    nn.PReLU(),
                                    nn.Conv3d(out_channels, out_channels, 3, stride=1, padding=1, groups=groups),
                                    Norm(dim=out_channels),
                                    nn.PReLU(),)
...
class ResidualSwinUnet3D(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, in_channel=4, num_classes=3, head_dim=32,
                 window_size: Union[int, List[int]] = 4, downscaling_factors=(2, 2, 2, 2, 2),
        ...
        self.enc12 = Encoder(in_dims=in_channel, hidden_dimension=hidden_dim, layers=layers[0],
                             ...
                             relative_pos_embedding=relative_pos_embedding)
        self.res12 = ResidualBlockEncDec(hidden_dim, hidden_dim)
        ...
        self.enc3 = Encoder(in_dims=hidden_dim, hidden_dimension=hidden_dim * 2,
                            ...
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        self.res3 = ResidualBlockEncDec(hidden_dim * 2, hidden_dim * 2)
        ...
        self.enc5 = Encoder(in_dims=hidden_dim * 4, hidden_dimension=hidden_dim * 8,
                            ...
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        self.res5 = ResidualBlockEncDec(hidden_dim * 8, hidden_dim * 8)
        
        
        self.enc6 = Encoder(in_dims=hidden_dim * 8, hidden_dimension=hidden_dim * 16,
                            layers=layers[4],
                            downscaling_factor=downscaling_factors[4], num_heads=heads[4],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        
        self.dec5 = Decoder(in_dims=hidden_dim * 16, out_dims=hidden_dim * 8,
                            layers=layers[3],
                            up_scaling_factor=downscaling_factors[4], num_heads=heads[3],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        ...
        self.converge5 = Converge(hidden_dim * 8)
    ...
    def forward(self, img):
        ...
        #print("x_s = ",x_s,", y_s = ",y_s,", z_s = ",z_s)
        #print("x_ws = ",x_ws,", y_ws = ",y_ws,", z_ws = ",z_ws)
        ...
        #print("\n shape of output of encoder12 = ",down12_1.shape) 
        res12 = self.res12(down12_1)
        
        
        
        down3 = self.enc3(down12_1)  # (B, 2C,X//8, Y//8, Z//8)
        res3 = self.res3(down3)
        
        down4 = self.enc4(down3)  # (B, 4C,X//16, Y//16, Z//16)
        
        down5 = self.enc5(down4)
        res5 = self.res5(down5)
        
        features = self.enc6(down5)  # (B, 8C,X//32, Y//32, Z//32)
        
        up5 = self.dec5(features)
        up5 = self.converge5(up5, res5)

        up4 = self.dec4(up5)  # (B, 8C, X//16, Y//16, Z//16 )
        # up1和 down3融合
        up4 = self.converge4(up4, down4)  # (B, 4C, X//16, Y//16, Z//16)

        up3 = self.dec3(up4)  # ((B, 2C,X//8, Y//8, Z//8)
        # up2和 down2融合
        up3 = self.converge3(up3, res3)  # (B,2C, X//8, Y//8)

        up12 = self.dec12(up3)  # (B,C, X//4, Y//4, Z// 4)
        # up3和 down1融合
        up12 = self.converge12(up12, res12)  # (B,C, X//4, Y//4, Z//4)
...
def swinUnet_t_3D_Residual(hidden_dim=64, layers=(2, 2, 2, 2, 2), heads=(3, 6, 9, 12,15), num_classes: int = 3, **kwargs):
    return ResidualSwinUnet3D(hidden_dim=hidden_dim, layers=layers, heads=heads, num_classes=num_classes, **kwargs)
